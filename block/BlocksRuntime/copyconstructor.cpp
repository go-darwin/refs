...
struct __testRoutine_block_impl_0 {
  ...
  TestObject one;
  __testRoutine_block_impl_0(void *fp, struct __testRoutine_block_desc_0 *desc, TestObject _one, int flags=0) : one(_one) {
...
static void __testRoutine_block_func_0(struct __testRoutine_block_impl_0 *__cself) {
  TestObject one = __cself->one; // bound by copy

    printf("my const copy of one is %d\n", one.version());
  }

static struct __testRoutine_block_desc_0 {
  size_t reserved;
  size_t Block_size;
} __testRoutine_block_desc_0_DATA = { 0, sizeof(struct __testRoutine_block_impl_0)};
void testRoutine() {
  TestObject one;

  void (*b)(void) = ((void (*)())&__testRoutine_block_impl_0((void *)__testRoutine_block_func_0, &__testRoutine_block_desc_0_DATA, one));
}
