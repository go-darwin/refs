...
struct __Block_byref_fiddly_0 {
  void *__isa;
__Block_byref_fiddly_0 *__forwarding;
 int __flags;
 int __size;
...
struct __main_block_impl_0 {
  ...
  __Block_byref_fiddly_0 *fiddly; // by ref
  __main_block_impl_0(void *fp, struct __main_block_desc_0 *desc, __Block_byref_fiddly_0 *_fiddly, int flags=0) : fiddly(_fiddly->__forwarding) {
...
static void __main_block_func_0(struct __main_block_impl_0 *__cself) {
  __Block_byref_fiddly_0 *fiddly = __cself->fiddly; // bound by ref

    int i;
    for (i = 0; i < 30; i++) {
      (fiddly->__forwarding->fiddly).ps[i]++;
      (fiddly->__forwarding->fiddly).qs[i]++;
    }
  }
static void __main_block_copy_0(struct __main_block_impl_0*dst, struct __main_block_impl_0*src) {_Block_object_assign((void*)&dst->fiddly, (void*)src->fiddly, 8/*BLOCK_FIELD_IS_BYREF*/);}

static void __main_block_dispose_0(struct __main_block_impl_0*src) {_Block_object_dispose((void*)src->fiddly, 8/*BLOCK_FIELD_IS_BYREF*/);}
...
int main(int argc, const char* argv[]) {
  __attribute__((__blocks__(byref))) __Block_byref_fiddly_0 fiddly = {(void*)0,(__Block_byref_fiddly_0 *)&fiddly, 33554432, sizeof(__Block_byref_fiddly_0), __Block_byref_id_object_copy_131, __Block_byref_id_object_dispose_131};
;
  BobTheStruct copy;

  void (*incrementFiddly)() = ((void (*)())&__main_block_impl_0((void *)__main_block_func_0, &__main_block_desc_0_DATA, (__Block_byref_fiddly_0 *)&fiddly, 570425344));

  memset(&(fiddly.__forwarding->fiddly), 0xA5, sizeof((fiddly.__forwarding->fiddly)));
  memset(&copy, 0x2A, sizeof(copy));

  int i;
  for (i = 0; i < 30; i++) {
    (fiddly.__forwarding->fiddly).ps[i] = i * i * i;
    (fiddly.__forwarding->fiddly).qs[i] = -i * i * i;
  }

  copy = (fiddly.__forwarding->fiddly);
  ((void (*)(__block_impl *))((__block_impl *)incrementFiddly)->FuncPtr)((__block_impl *)incrementFiddly);

  if (&copy == &(fiddly.__forwarding->fiddly)) {
  ...
  for (i = 0; i < 30; i++) {
    ...
    if (((fiddly.__forwarding->fiddly).ps[i] != copy.ps[i] + 1) || ((fiddly.__forwarding->fiddly).qs[i] != copy.qs[i] + 1)) {
