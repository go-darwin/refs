...
int main(int argc, char* argv[]) {
  ...
  if (child == 0) {
    ...
    if (fd != 1) {
      fprintf(__stderrp, "didn't open custom error file %s as 1, got %d\n", buffer,
              fd);
      exit(1);
    }
    close(2);
  ...
  if (errorfile) {
    ...
    if (!gotErrorFile && gotOutput) {
      printf(
          "didn't read errorfile %s, it should have something "
          "from\n*****\n%s\n*****\nin it.\n",
          errorfile, got);
      exit(1);
