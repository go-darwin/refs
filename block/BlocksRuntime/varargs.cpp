...
static int __main_block_func_0(struct __main_block_impl_0 *__cself, int n, ...) {

    int result = 0;
    va_list numbers;
    int i;

    __builtin_va_start(numbers, n);
    for (i = 0; i < n; i++) {
      result += __builtin_va_arg(numbers, int);
    }
    __builtin_va_end(numbers);
...
int main(int argc, const char* argv[]) {
  int (*sumn)(int n, ...) = ((int (*)(int, ...))&__main_block_impl_0((void *)__main_block_func_0, &__main_block_desc_0_DATA));
  int six = ((int (*)(__block_impl *, int))((__block_impl *)sumn)->FuncPtr)((__block_impl *)sumn, 3, 1, 2, 3);
