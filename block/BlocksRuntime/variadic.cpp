...
static long __main_block_func_0(struct __main_block_impl_0 *__cself, const char *format, ...) {
    ...
    for (p = format; *p; p++) switch (*p) {
        case 'i':
          ...
          break;
        case 'd':
          d = __builtin_va_arg(argp, double);

          result += (int)d;
          break;
        case 'c':
          c = __builtin_va_arg(argp, int);
...
int main(int argc, char* argv[]) {
  long (*addthem)(const char*, ...) = ((long (*)(const char *, ...))&__main_block_impl_0((void *)__main_block_func_0, &__main_block_desc_0_DATA));
  long testresult = ((long (*)(__block_impl *, const char *))((__block_impl *)addthem)->FuncPtr)((__block_impl *)addthem, "ii", 10, 20);
  if (testresult != 30) {
    printf("got wrong result: %ld\n", testresult);
    return 1;
  }
  testresult = ((long (*)(__block_impl *, const char *))((__block_impl *)addthem)->FuncPtr)((__block_impl *)addthem, "idc", 30, 40., 'a');
  if (testresult != (70 + 'a')) {
