...
- (bool)compileUnlessExists:(bool)skip {
  ...
  char* argv[argc + 1];
  for (int i = 0; i < argc; ++i) argv[i] = (char*)[compileLine pointerAtIndex:i];
  argv[argc] = NULL;
  pid_t child = fork();
  if (child == 0) {
    execv(argv[0], argv);
...
- (void)addLibrary:(const char*)dashLSomething {
  if (!extraLibraries) {
    extraLibraries = [NSPointerArray pointerArrayWithOptions:NSPointerFunctionsStrongMemory |
...
- (TestFileExe*)lineForOptions:(int)options {  // nil if no can do
  ...
  if (options & DoDashO)
    [pa addPointer:"-O"];
  else if (options & DoDashO2)
    [pa addPointer:"-O2"];
  else if (options & DoDashOs)
    [pa addPointer:"-Os"];
...
+ (TestFileExeGenerator*)generatorFromFilename:(char*)filename {
  ...
  } else if (!strcmp(ext, ".cc") || !strcmp(ext, ".cp") || !strcmp(ext, ".cxx") ||
...
- (NSString*)description {
  return [NSString stringWithFormat:@"%s: %s%s%s%s%s%s", filename, LOOKFOR, hasGC ? " GC" : "",
                                    hasRR ? " RR" : "", hasCPlusPlus ? " C++" : "",
                                    wantsC99 ? "C99" : "", supposedToNotCompile ? " FAIL" : ""];
}
...
void printDetails(NSArray* failures, const char* whatAreThey) {
  if ([failures count]) {
    ...
    for (TestFileExe* line in failures) {
      ...
      if (radar) printf(" (due to %s?),", radar);
...
void help(const char* whoami) {
  ...
  printf("     -dyld p            override DYLD_LIBRARY_PATH and DYLD_FRAMEWORK_PATH to p when "
         "running tests\n");
  printf("     <compilerpath>     directory containing gcc-4.2 (or clang) that you wish to use to "
         "compile the tests\n");
  printf("     --                 assume stdin is a grep CON"
         "FIG across the test sources\n");
  printf("     otherwise treat each remaining argument as a single test file source\n");
  printf("%s will compile and run individual test files under a variety of compilers, c, obj-c, "
         "c++, and objc++\n",
         whoami);
  ...
  printf("(actually all forms of extensions recognized by the compilers are honored, .cc, .c++ "
         "etc.)\n");
  printf("\nTest files should run to completion with no output and exit (return) 0 on success.\n");
  printf("Further they should be able to be compiled and run with GC on or off and by the C++ "
         "compilers\n");
  printf("A line containing the string CON"
         "FIG within the source enables restrictions to the above assumptions\n");
  printf("and other options.\n");
  printf("Following CON"
         "FIG the string\n");
  ...
  printf("Files that are known to exhibit unresolved problems can provide the term \"open\" and "
         "this can");
  printf("in turn allow highlighting of fixes that have regressed as well as identify that fixes "
         "are now available.\n");
  printf("Files that exhibit known bugs may provide\n");
  printf("    rdar://whatever such that if they fail the rdar will get cited\n");
  printf(
      "Files that are expected to fail to compile should provide, as their last token sequence,\n");
  printf("    error:\n");
  printf(" or error: substring to match.\n");
  printf("Files that are expected to produce a runtime error message should provide, as their last "
         "token sequence,\n");
  printf("    warning: string to match\n");
  printf("\n%s will compile and run all configurations of the test files and report a summary at "
         "the end. Good luck.\n",
         whoami);
...
int main(int argc, char* argv[]) {
  ...
  printf("\n--- results ---\n\n%lu successes\n%lu unexpected compile failures\n%lu failure to fail "
         "to compile errors\n%lu run failures\n",
         [successes count], [failureToCompile count], [failureToFailToCompile count],
         [failureToRun count]);
